from typing import Tuple

from flask import Blueprint, request, jsonify, Response, current_app

from app.models.input.user_input import UserInput
from app.services.user_service import UserService

user_blueprint = Blueprint("user", __name__, url_prefix="users")


@user_blueprint.post("")
def create() -> Tuple[Response, int]:
    user = UserInput(**request.json)
    service = current_app.config["INJECTOR"].get(UserService)
    return jsonify(service.create(user).model_dump()), 201


@user_blueprint.get("")
def get_all() -> Tuple[Response, int]:
    service = current_app.config["INJECTOR"].get(UserService)
    return jsonify([user.model_dump() for user in service.get_all()]), 200


@user_blueprint.get("/paginated")
def get_paginated() -> Tuple[Response, int]:
    page = int(request.args.get("page", 1))
    per_page = int(request.args.get("per_page", 10))
    service = current_app.config["INJECTOR"].get(UserService)
    return jsonify(service.get_paginated(page, per_page)), 200


@user_blueprint.get("/<user_id>")
def get(user_id) -> Tuple[Response, int]:
    service = current_app.config["INJECTOR"].get(UserService)
    return jsonify(service.get(user_id).model_dump()), 200


@user_blueprint.put("/<user_id>")
def update(user_id) -> Tuple[Response, int]:
    service = current_app.config["INJECTOR"].get(UserService)
    return jsonify(service.update(user_id, UserInput(**request.json)).model_dump()), 200


@user_blueprint.delete("/<user_id>")
def delete(user_id) -> Tuple[Response, int]:
    service = current_app.config["INJECTOR"].get(UserService)
    service.delete(user_id)
    return jsonify(), 204
